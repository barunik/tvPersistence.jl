...
function TVAR_forecast(
    ...
    for ii in 0:(fcast_length-1)
        ...
        h_fore = forecast_tvAR(sample, p, kernel_width, horizon; tkernel=kernel_type)
        forecasts[ii+1] = mean(h_fore)
        idx = train_end+1 : train_end+horizon
        realized[ii+1] = mean(y[idx])
        errors[ii+1]   = forecasts[ii+1] - realized[ii+1]
...
function TVHAR_forecast(
    ...
    for ii in 0:(fcast_length-1)
        ...
        result = tvOLS(Rmat, Y, kernel_width, kernel_type)
...
function EWD_forecast(
    ...
    for ii in 0:(fcast_length-1)
        ...
        for j in 1:JMAX
            ...
            g_scales[j] = compute_g_scale(length(eps_scale_j), beta_scale_j, eps_scale_j, KMAX, j)[KMAX-2^j+1:end]
        end
