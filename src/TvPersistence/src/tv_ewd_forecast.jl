...
Compute the matrix of shifted multiscale impulse responses values needed for TV-EWD forecasting (As in Ortu et. al 2020).
...
- Dyadic decomposition via scale components follows the Extended Wold decomposition logic.
    ...
    for ii in 0:(fcast_length-1)
        ...
        if !(user_specified_scales == nothing)
            ...
            for j in user_specified_scales
                ...
                g_scale_j = compute_g_scale(length(epsilon_scale_j), IRF_beta_scale_j, epsilon_scale_j, KMAX, j)[g_scale_start_index:end]
        ...
        # Option 2. Calculate all scale components
        else
            # For each scale component, calculate the beta coefficients and scale components x^{j}_t
            for j in 1:JMAX
