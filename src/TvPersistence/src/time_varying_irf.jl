...
function compute_residuals_tv(response_data, regressor_data, coefficients::Matrix{Float64})::Vector{Float64}
...
Compute time-varying impulse response functions (IRFs) using local linear estimation.
...
- `kernel_type::String`: Kernel type to use ("Gaussian", "Epanechnikov", "one-sided", "triweight").
...
function IRFalpha_tvp(
    ...
    coeffs_tvp = tvOLS(x, y, kernel_width, kernel_type).coefficients

    # Compute residuals for each time step
    residuals_tvp = compute_residuals_tv(y, x, coeffs_tvp)
